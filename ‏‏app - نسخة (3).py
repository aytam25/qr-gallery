...
@login_required
def upload_images():
    """رفع صور متعدد إلى Google Drive"""
    if request.method == 'POST':
        ...
        for file in files:
            if file and allowed_file(file.filename):
                try:
                    ...
                    # 2. رفع الصورة الأصلية إلى Google Drive
                    drive_url = upload_to_drive(temp_path, new_filename)
                    
                    if drive_url:
                        # 3. رفع الصورة المصغرة إلى Google Drive
                        thumb_drive_url = upload_to_drive(thumb_path, thumb_filename)
                        ...
                        # 5. حفظ في قاعدة البيانات باستخدام روابط Google Drive
                        new_image = Image(
                            ...
                            image_url=drive_url,  # رابط Google Drive للصورة الأصلية
                            thumbnail_url=thumb_drive_url or drive_url,  # رابط Google Drive للصورة المصغرة
                            category_id=category_id,
                            uploaded_by=current_user.id,
                        ...
                        print(f"✅ تم رفع {filename} إلى Google Drive")
                    else:
                        failed.append({'file': filename, 'error': 'فشل رفع الملف إلى Google Drive'})
        ...
        flash(f'✅ تم رفع {len(uploaded)} صورة إلى Google Drive بنجاح', 'success')
        if failed:
